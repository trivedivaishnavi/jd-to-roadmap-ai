...
The AI roadmap kept cutting off halfway through (Week 5 would just stop mid-sentence) — turned out I'd set too low a limit on how much the AI was allowed to write (max_tokens). Bumped it way up from 900 to 2000 and now it finishes the whole thing, including the personalized LinkedIn/GitHub tips section at the end. Also fixed an earlier issue where the roadmap was showing literal asterisks (*) instead of proper bold text and bullet points — turns out the AI writes in Markdown format, so I had to add a library to actually convert that into real HTML before displaying it.

Added resume upload as a third way to detect known skills, alongside GitHub and the manual checkboxes. Used pdfplumber to pull text out of an uploaded PDF, then ran it through the same skill-matching function I already had for job descriptions — nice that I could reuse that instead of writing something new. Had to remember to add enctype="multipart/form-data" to the form tag, otherwise file uploads don't work in Flask (didn't know that was a thing until it silently failed).

Added a proper hero section to the homepage with an icon and a clearer headline instead of just a plain title — makes it feel more like a real product than a form. Also added a circular percentage indicator on the results page showing your overall skill match, using a CSS conic-gradient trick instead of an actual chart library, which was a fun small thing to figure out.

Ngl, today felt like a lot — kept adding feature after feature (hero section, match percentage, loading animation) and at some point it just felt like too much to keep track of. Taking a short breather. Still proud of how much this project has grown though, from a single "hello world" route to something that actually looks and feels like a real app.
Spent way too long today wondering why my bar chart just wasn't showing up — no error on the page, just... nothing. Turned out Chart.js was loading after my script tried to use it, so it broke silently. Moved the script tag to the head section and it finally worked. Small fix, annoying to find.
...
Added a little custom SVG illustration on the homepage — a dotted path with dots and a flag at the end, kind of representing the whole "roadmap" idea visually. Didn't want to use random stock images so just drew this myself directly in the HTML using SVG, no actual image file needed.
Chart was way too huge, filling the whole screen. Turns out Chart.js just does that unless you box it in with a fixed height. Fixed.
Skill names on the chart were all slanted and overlapping, no room for them side by side. Switched to horizontal bars instead so they actually sit flat and readable. Matched the colors to the pink theme too while I was at it.